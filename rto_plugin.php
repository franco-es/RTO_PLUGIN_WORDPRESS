  ...
  if (isset($_GET['selected_product_id']) && !empty($_GET['selected_product_id'])) {
    ...
    if ($bookings_ids) {
      ...
      foreach ($bookings_ids as $booking_id) {
        ...
        if ($order) {
          $nombre = ('' !== $order->get_billing_first_name() ? $order->get_billing_first_name() : 'N/A');
          $apellido = ($order->get_billing_last_name() ? $order->get_billing_last_name() : 'N/A');
          $nro_turno = ('' !== $order->get_billing_company() ? $order->get_billing_company() : 'N/A');
          $patente = ('' !== $order->get_billing_address_2() ? $order->get_billing_address_2() : 'N/A');
          ...
          $nro_turno = $patente = $customer_mail = $customer_phone = $nombre = $apellido = 'N/A';
        }
